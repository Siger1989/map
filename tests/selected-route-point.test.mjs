...
test('map selection moves the profile cursor and shows grade, elevation and recorded speed, including reverse and missing data',async(t)=>{
  ...
  const cursor=()=>[...document.querySelectorAll('[aria-label="所选路线点"] path')].find(path=>path.getAttribute('fill')==='#ffad55');
  const cursorX=()=>Number(cursor()?.getAttribute('d').match(/^M\s*(-?[\d.]+)/)?.[1]);
  const info=()=>document.querySelector('[aria-label="剖面所选点信息"]')?.textContent;
  await render(point(.0005));const first=cursorX();assert.ok(Number.isFinite(first));
  assert.match(info(),/56 米/);assert.match(info(),/110m/);assert.match(info(),/路段坡度 \+10\.2°/);assert.match(info(),/速度 6\.7 km\/h/);
  await render(point(.0015));assert.ok(cursorX()>first);assert.match(info(),/路段坡度 -5\.1°/);
  await render(point(.0005),{reversed:true});assert.ok(cursorX()>first);assert.match(info(),/167 米/);assert.match(info(),/路段坡度 -10\.2°/);
  ...
  await render({...point(.001),trackId:'other'});assert.equal(cursor()??null,null);assert.equal(info(),undefined);
