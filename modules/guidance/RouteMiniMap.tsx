...
export function RouteMiniMap({coordinates,color='#c2513f',routes,selectedId='original',settings}: {
  ...
  const camera=useRef<import('maplibre-gl').Map|null>(null);
  const [view,setView]=useState({zoom:0,pitch:0,bearing:0});
  ...
  return <section className="route-mini-overview" aria-label="路线缩略图：选中路线高亮，其他方案淡色显示">
    <div ref={container} className="route-mini-map"/><span className="route-mini-north">北 <span style={{display:'inline-block',transform:`rotate(${-view.bearing}deg)`}}>↑</span></span>
    <div className="route-mini-controls" aria-label="导航小地图控制">
      <button type="button" aria-label="放大导航小地图" onClick={()=>camera.current?.zoomIn({duration:180})}>＋</button>
