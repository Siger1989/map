...
export function RallyNavigation({ route, guidance: g, fix, onNormal, onStart, onStop, onLocate, onOverview, display }: {
  ...
  useEffect(() => {
    const block = top.current, root = block?.closest<HTMLElement>('.observatory');
    if (!block || !root) return;
    const update = () => root.style.setProperty('--rally-map-top', `${block.offsetTop + block.offsetHeight + 4}px`);
    const observer = new ResizeObserver(update);
    observer.observe(block);
    update();
    return () => { observer.disconnect(); root.style.removeProperty('--rally-map-top'); };
  }, []);
  ...
  return <div className="rally-workspace" aria-label="拉力路书模式">
    ...
    <div ref={top} className="rally-top">
    <section className="rally-roadbook" aria-label="连续转向提示">
      <div className="rally-cue" data-kind={currentKind}><div><small>{located ? '当前路段' : '路线预览'}</small><Icon aria-hidden="true" /></div><div className="rally-cue-text"><span className="rally-distance-label">{currentKind === 'finish' ? '距终点' : located ? '前方' : '距起点'}</span><strong data-long={formatDistance(first.distance).length > 6}>{formatDistance(first.distance)}</strong></div><h3 data-long={first.text.length > 24}>{first.text}</h3></div>
