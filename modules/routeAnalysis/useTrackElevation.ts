...
export function useTrackElevation(
  target: ManualTrack | null,
  enabled: boolean,
) {
  const key = useMemo(
    () =>
      target
        ...
        : '',
    [target?.id, target?.segments, target?.samples],
  );
    ...
    const timer = setTimeout(() => {
      ...
      void readProfile(points, request.signal)
        .then((heights) => {
          ...
          const available = heights.filter((p) =>
            finiteHeight(p.elevation),
          ).length;
          const error =
          ...
          setResult({
            ...
            profile: terrainProfileTrack(target, heights),
            estimated: available > 0,
            error,
          });
        })
        .catch(() => {
