...
export function folderPath(layout: CollectionLayout, id: string): string {
  ...
  return group ? `${group.parentId ? `${folderPath(layout, group.parentId)} / ` : ''}${group.name}` : '未分组';
}
...
export function folderContents(layout: CollectionLayout, entries: ReturnType<typeof folderEntries>, id: string | null) {
...
export function collectionSubset(layout: CollectionLayout, entries: ReturnType<typeof folderEntries>): CollectionLayout {
  ...
  return {
    ...
    assignments: Object.fromEntries(entries.map((e) => [e.key, groupFor(layout, e).id])),
    order: layout.order.filter((key) => keys.has(key)),
    ...(layout.treeOrder && { treeOrder: layout.treeOrder.filter(key => keys.has(key) || (key.startsWith('folder:') && (groups.has(key.slice(7)) || key === 'folder:unfiled'))) }),
  };
}
