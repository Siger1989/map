...
框选完成进入独立的 `BoxSelectionResults`，只展示本次框选对象；可取消勾选、导出、删除，并在当前结果窗口撤销删除。删除与撤销都复用 `saveWorkbench` 的并发检查，不覆盖其他页面的新数据。删除后的空态收起为短提示和操作行，不进入完整收藏目录。普通收藏目录隐藏上方地图搜索及工具覆盖，使用浅色目录行并为下方地图保留空间。
