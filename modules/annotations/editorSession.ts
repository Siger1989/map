...
export function annotationEditItems(
  items: Annotation[],
  edit: AnnotationEdit | null,
) {
  if (!edit) return items;
  const found = items.some((item) => item.id === edit.draft.id);
  return found
    ? items.map((item) => (item.id === edit.draft.id ? edit.draft : item))
    : [...items, edit.draft];
}
...
export function patchAnnotation(item: Annotation, patch: Partial<Annotation>) {
  if (
    item.trackAnchor &&
    patch.coordinates &&
    patch.coordinates.some((n, i) => n !== item.coordinates[i])
  )
  ...
  if (
    ...
    throw new Error('这个标记已绑定剖面，请在剖面编辑中沿线移动。');
...
export function positionOffsets(
  ...
  const origin = mercator(base.coordinates),
    now = mercator(pose.coordinates);
  let dx = now.x - origin.x;
  dx -= Math.round(dx); // shortest crossing of the antimeridian
  return new Vector3(
    dx / origin.unit,
    (origin.y - now.y) / origin.unit,
    pose.altitude - base.altitude,
  )
    .applyQuaternion(new Quaternion().fromArray(pose.rotation).invert())
    .toArray();
}

export function withPositionOffset(
  base: Pose,
  pose: Pose,
  axis: 0 | 1 | 2,
  value: number,
  ...
  if (!Number.isFinite(value) || Math.abs(value) > 100000)
    throw new Error('位移需在 ±100000 米以内');
  const values = positionOffsets(base, pose);
  values[axis] = value;
  const delta = new Vector3(...values).applyQuaternion(
    new Quaternion().fromArray(pose.rotation),
  );
  const origin = mercator(base.coordinates);
  const ll = coordinate(
    origin.x + delta.x * origin.unit,
    origin.y - delta.y * origin.unit,
  );
  ...
  return {
    ...pose,
    coordinates: [((((ll[0] + 180) % 360) + 360) % 360) - 180, ll[1]],
    altitude,
  };
}
...
export function editorPose(item: Annotation) {
  ...
  return pose && item.kind === 'pin'
    ? { ...pose, rotation: [0, 0, 0, 1] as Pose['rotation'] }
    : pose;
}
