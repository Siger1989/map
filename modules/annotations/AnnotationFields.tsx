  ...
  if (picker)
    return (
      <div className="marker-picker">
          ...
          {picker === 'icon'
            ? Object.entries(MARKER_ICONS).map(([id, icon]) => (
                <button
                  ...
                  onClick={() => {
                    if (change({ icon: id as Annotation['icon'] }))
                      setPicker(null);
                  }}
                >
                  <AnnotationIcon item={{ icon: id as Annotation['icon'] }} />
            ...
            : colors.map(([color, name]) => (
                <button
                  ...
                  onClick={() => {
                    if (change({ color })) setPicker(null);
                  }}
                >
        ...
        {picker === 'color' && (
          <label className="marker-custom-color">
            自定义颜色
            <input
              type="color"
              aria-label="自定义标记颜色"
    ...
    <>
      ...
      <div className="marker-style-row">
        ...
        <button onClick={() => setPicker('color')}>
          ...
          <span>{colors.find(([c]) => c === item.color)?.[1] ?? '自定'}</span>
          <ChevronDown size={14} />
        </button>
      </div>
      ...
      {item.kind !== 'pin' && (
        <details className="marker-model-options">
          <summary>模型尺寸与显示</summary>
            ...
            {(
              [
                ...
                ...(item.kind === 'box' || item.kind === 'prism'
                  ? [['length', '长度 m']]
                  : []),
                ...(item.kind !== 'sphere' ? [['height', '高度 m']] : []),
                [
                  'offset',
                  item.placement === 'underground' ? '埋深 m' : '离地 m',
                ],
                ['opacity', '不透明度'],
              ] as [keyof Annotation, string][]
            ).map(([key, label]) => (
              <label key={key}>
                <span>{label}</span>
                <NumericValue
                  ...
                  min={key === 'offset' ? 0 : 0.1}
                  max={key === 'opacity' ? 0.85 : 10000}
                  change={(n) => change({ [key]: n })}
                  reset={() => change({ [key]: base[key] })}
                />
              </label>
            ))}
            <label>
              ...
              <select
                ...
                onChange={(e) =>
                  change({
                    placement: e.target.value as Annotation['placement'],
                  })
                }
              >
          ...
          <label className="marker-check">
            <input
              type="checkbox"
              checked={item.terrainCut ?? item.placement === 'underground'}
...
export function MarkerPosition({
  ...
  base: Annotation;
  change: Change;
  transform: (item: Annotation) => boolean;
  origin?: Pose | null;
}) {
  const pose = editorPose(item),
    origin = editOrigin ?? editorPose(base);
  const [error, setError] = useState('');
  if (!pose || !origin)
    return <p role="status">请先在详情中读取或填写地面海拔，再调整模型。</p>;
  const offsets = positionOffsets(origin, pose),
    angles = rotationDegrees(pose);
  const axes = item.kind === 'pin' ? ([0, 1] as const) : ([0, 1, 2] as const);
  const apply = (axis: 0 | 1 | 2, value: number, rotation = false) => {
    try {
      const next = rotation
        ? withRotationAxis(pose, axis, value)
        : withPositionOffset(origin, pose, axis, value);
      ...
      setError(e instanceof Error ? e.message : '位置超出支持范围');
    }
  };
  return (
    <div className="marker-position" data-kind={item.kind}>
      <div className="marker-axis-heading">
        <span />
        <div>
          {axes.map((axis) => (
            <b key={axis} data-axis={axis}>
              {['X', 'Y', 'Z'][axis]}
            </b>
          ))}
        </div>
      </div>
      {[false, ...(item.kind === 'pin' ? [] : [true])].map((rotation) => (
        <div key={String(rotation)} className="marker-axis-group">
          <span className="marker-axis-unit">
            {rotation ? '旋转 °' : '位移 m'}
          </span>
          <div className="marker-axis-values">
            {axes.map((axis) => (
              <div key={axis} data-axis={axis}>
                <NumericValue
                  label={`${['X', 'Y', 'Z'][axis]}${rotation ? '旋转角度' : '位移'}`}
                  value={rotation ? angles[axis] : offsets[axis]}
                  min={rotation ? -360 : -100000}
                  max={rotation ? 360 : 100000}
                  step={rotation ? 0.1 : 0.01}
                  change={(n) => apply(axis, n, rotation)}
                  reset={() => apply(axis, 0, rotation)}
                />
              </div>
            ))}
          </div>
      ...
      {error && (
        <p role="alert" className="marker-error">
          {error}
        </p>
      )}
    </div>
...
export function MarkerData({
  ...
  return (
    <div className="marker-data">
      <div className="marker-data-heading">
        <span>自定义属性 · {fields.length}</span>
        <button
      ...
      {fields.map((field, i) => (
        <div className="marker-attribute-row" key={i}>
          <SmartInput
          ...
          <button
            aria-label={`删除属性 ${i + 1}`}
            onClick={() =>
              change({ attributes: fields.filter((_, n) => n !== i) })
            }
          >
            <Trash2 size={16} />
          </button>
      ...
      <label className="marker-inline-field marker-note">
        <span>备注</span>
        <SmartTextarea
          aria-label="标记备注"
          rows={1}
...
export function MarkerCoordinates({
  ...
  return (
    <div className="marker-coordinate-fields">
      {(
        [
          ['经度', 0, -180, 180],
          ['纬度', 1, -85, 85],
        ...
        <label key={index}>
          ...
          <NumericValue
            ...
            change={(n) => {
              const coordinates = [
                ...item.coordinates,
              ] as Annotation['coordinates'];
              coordinates[index] = n;
              change({ coordinates, groundElevation: null });
            }}
            reset={() => {
              const coordinates = [
                ...item.coordinates,
              ] as Annotation['coordinates'];
              coordinates[index] = base.coordinates[index];
              change({ coordinates, groundElevation: null });
            }}
          />
        </label>
      ...
      <label>
        ...
        <NumericValue
          ...
          min={-12000}
          max={10000}
          change={(n) => change({ groundElevation: n })}
          reset={() => change({ groundElevation: base.groundElevation })}
        />
      </label>
      ...
      <button onClick={refresh} disabled={reading}>
        {reading ? '读取地形…' : '重新读取地形海拔'}
      </button>
    </div>
  );
}
