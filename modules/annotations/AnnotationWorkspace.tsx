...
import { readRegions, REGION_STORAGE, coordinateKey } from '../collections/regions';
import { AnnotationIcon, MarkerBasic, MarkerCoordinates, MarkerData, MarkerPosition } from './AnnotationFields';
...
export function AnnotationWorkspace({ state, shownItem, tab, onTab, onClose, onNavigate, onShare, dragging, terrainStatus, photos = [], onCapture, onPhoto, cameraStatus, cameraBusy, cameraRetry, onCameraRetry }: {
  ...
  const back = () => {
    if (confirm || state.selectionRequest) { setConfirm(null); state.resolveSelection(false); return; }
    if (editing) {
      if (coordinates) { setCoordinates(false); return; }
      if (state.dirty) setConfirm('leave');
      else { state.cancelEdit(); setView('summary'); }
  ...
  useEffect(() => {
    ...
    const keepInputVisible = () => {
      ...
      frame = requestAnimationFrame(() => {
        ...
        if (input instanceof HTMLElement && root.current?.contains(input) &&
  ...
  useEffect(() => {
    const escape = (event: KeyboardEvent) => {
      ...
      event.preventDefault(); event.stopPropagation(); latestBack.current();
    };
  ...
  const startEdit = (precise = false) => { state.beginEdit(item.id); onTab(precise ? 'position' : 'basic'); setCoordinates(precise); };
  ...
  let place = '地点标记';
  try {
    const region = readRegions(localStorage.getItem(REGION_STORAGE))[`annotation:${item.id}`];
    if (region?.coordinateKey === coordinateKey(item.coordinates))
      place = [region.city, region.township || region.district].filter(Boolean).join(' · ') || region.province || place;
    else place = `${item.coordinates[1].toFixed(4)}°, ${item.coordinates[0].toFixed(4)}°`;
  } catch { /* Exact coordinates remain in details if region cache is unavailable. */ }
  return <section ref={root} className="marker-workspace" aria-label={editing ? '编辑标记' : view === 'details' ? '标记详情' : '标记摘要'}
    data-view={editing ? tab : view} data-dragging={dragging} onKeyDown={(e) => {
      if (e.key === 'Escape') { e.preventDefault(); e.stopPropagation(); back(); }
      if (e.key === 'Tab' && (confirm || state.selectionRequest)) {
        const buttons = root.current?.querySelectorAll<HTMLButtonElement>('.marker-confirm button');
    ...
    {editing ? <header className="marker-editor-bar">
      <button aria-label="返回标记摘要" onClick={back}><ChevronLeft size={20} /></button>
      <nav aria-label="标记编辑分组">{([['basic', '基本'], ['position', '位置'], ['data', '资料']] as const).map(([id, name]) =>
        <button key={id} aria-pressed={tab === id} onClick={() => { onTab(id); setCoordinates(false); }}>{name}</button>)}</nav>
      <button className="marker-save" onClick={() => { if (state.saveEdit()) { state.rememberAttributes(item.id); setView('summary'); } }}>保存</button>
    </header> : view === 'summary' ? <>
      <header className="marker-summary-title"><AnnotationIcon item={item} /><strong title={item.name}>{item.name || '未命名'}</strong>
        <button className="marker-delete" aria-label="删除标记" onClick={() => setConfirm('delete')}><Trash2 size={16} /></button>
        <button aria-label="关闭标记" onClick={onClose}><X size={20} /></button>
      </header>
      <div className="marker-summary-place"><span title={place}>{place}</span><span>{item.groundElevation === null ? '海拔 —' : `海拔 ${Number(item.groundElevation.toFixed(1))} m`}</span></div>
    ...
    {!editing && view === 'details' && <div className="marker-scroll marker-details">
      <h3>{item.name || '未命名'}</h3>
