...
- 最新要求：**每次提供当前层级的效果预览图，用户确认后才进入下一层**。用户追加一级小号删除入口，已用image_gen在关闭按钮左侧添加“删除”，生成docs/visuals/marker-level-one-preview-v2.png，说明与提示词见同名.md，v1保留。当前等待一级v2视觉确认，不开始二级设计或业务实现；仍无需滚动。图片为效果图，不是运行截图，未实现或执行删除。
