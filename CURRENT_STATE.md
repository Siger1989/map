...
- 最终430/430与types PASS；新ZIP57034622字节/SHA256 38f9e3ebe0ce926b82d5489a39bd1a88d87f4cdecba76d6448ab9fd62e02ea9d，535文件/473地形/Mac执行位PASS。实际包18787启动两次复用/退出0/HTTP200，验证进程已关闭。
- 003df933源码已推送核对；正在补充本修正提交。GitHub draft id386053622仍未公开，初次APK及旧ZIP大文件上传尚未完成；完成后必须用新ZIP与校验文件替换再发布。APK代码/资源未受此电脑服务修正影响。
...
- 新增路线连接保色：tracks/edgeColors 为各物理边保存颜色，连接/合并/反向/加点/移动/删点与草稿撤销/保存及 TrackLayer 适配；新增12项定向检查已过，继续全量检查。连接空隙用起始路线颜色；用户主动统一改色才清除分段颜色。测量和剖面不因此改动。
- 手机复验发现分类拖动与后台省市归类写入产生存档冲突，已改标签排序只比较标签字段并合入最新快照；地区/分组保存派发数据变更事件，待再次实际拖动确认。
- 用户确认方向：地图 A/B 定义竖直剖面，后续点约束在 AB 及延长线；工程图上剖面下等高线平面图。测量计算/选点保持现状；测量与剖面均进入收藏，顶部分类 TAG 可拖动排序并持久化。
- 当前分支 codex/sync-20260910，跟踪 origin/codex/huawei-webview-touch；pull 后基线 945ede4，工作区原本干净。此前 43 个草稿在 stash 0c704442 与本地 .openai/sync-20260910-091832-backup 保留。
- 实施范围：collections 目录/分类排序、measurement 保存适配、section 勘探线及平剖图、outdoor 备份和 app/page 接线。原轨迹、照片、导航计算、签名与 Logo 不改。跨模块只通过类型、props 和数据变更事件连接；新可选字段兼容原 version1，旧剖面与测量存档保留。
- 已实现初稿：测量收藏/导入导出删除撤销、标签长按拖动排序；surveyLine 几何、同一 DEM 网格采样、工程平剖图、地图手柄、沿线标记和图纸信息表单。新增模块均在 modules/section/survey* 与 Survey*、collections/CollectionTabs/tabOrder，原模块按适配范围接线。
- 用户追加并确认：A/B 可手点或选已有标记；编辑可选调整方向或仅沿线滑动，其他点默认共线。转向时 C/D 和沿线标记保持距 A 里程，跟随转动并重新采样高程；不移动用于选点的原始标记。后期可加标记，沿用名称/备注/照片编辑。图纸照第二张工程图，包含项目/图名/图号/编制/审核/日期/补充来源/备注录入。
- 曾实现的竖井初稿：独立窗口左侧井柱，右侧只填每层结束深度/岩性/颜色；历史名称自动带色纹。按用户最新指示完整移至本地草稿，未验证完成、未发布。
- 补全：点名称/备注、附加点移除与标记解绑定、A/B 照片标记取点、DEM刷新、剖面列表删除/撤销关联、导入标记/剖面ID联合重映射。类型检查已过；395项其余逻辑通过（暂停前400项仅1项竖井附表文字断言失败，已随草稿移出）；竖井撤出后需重验。390×844浏览器实际创建A/B、DEM、平剖图初验PASS，更多编辑与360尺寸待验。
- 验证：新增 9 项几何/共线/转向里程/DEM等高线缺测/收藏备份/ID冲突/排序检查全部通过；初轮 types 通过。尚未完整浏览器、全量逻辑或最终构建。局部日志 .openai/tsc-0223-* 与 tests-0223-focused.log。
- 待完善：标记/剖面联合导入ID重映射、图纸长资料完整排版、附加点删除与资料编辑、移动端手势/面板遮挡、地形重试刷新。之后全量检查、390/360真实浏览器、版本提升/新APK/签名资源核验、源码与Release推送。未生成新 APK/未发布；HarmonyOS6.1原生限制仍同既有说明。
