...
- 本次只整理同步配置、忽略规则和说明；业务模块与0.1.2已验证的APK不改。代码/测试/473地形PNG同步到Git；APK/校验/安装说明发布到测试版Release；Token、签名密钥、依赖及日志排除。
- 初步验证：GitHub目标仓库可写（ADMIN），git push --dry-run成功。正式提交/推送及Release上传正在完成；最终核验后补充结果。

## 最新交付：2026-09-05，0.1.2-test
