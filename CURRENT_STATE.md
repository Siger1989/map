...
- 最新要求：**每次提供当前层级的效果预览图，用户确认后才进入下一层**。已用image_gen完成一级摘要卡预览docs/visuals/marker-level-one-preview-v1.png，来源与提示词见同名.md；当前等待一级视觉确认，不开始二级设计或业务实现。一级内容无需滚动；图片为生成式效果图，不是运行截图。
