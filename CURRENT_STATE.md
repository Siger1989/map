# 当前状态 — 2026-09-24 / 0.2.71-test 待新增修正并重建

## 2026-09-24 0.2.71-test Android 测试包

此前工作区源码已构建过一个临时的0.2.71-test/code78 APK，沿用包名 `com.guanyun.weather.shantu.preview` 和原 `4a94` 证书：`APK/Shantu-0.2.71-test-standalone.apk`，57,838,684 bytes，SHA-256 `41b41ad6674510bc8b7410661235435148fd1e558196457020bbe6a55897ce44`；该次构建的v2/v3签名、zipalign和473张地形瓦片核验通过。用户随后追加本轮问题，最终实现后必须重建此 APK；当前文件不代表最终交付包。

- 本版修复框选双指平移/缩放/旋转手势（根据真机报告调整旋转方向）；收藏夹统一导入文件与标记Excel；将分享入口集中为JSON/XLSX选择并移除独立Excel导出；互斥不同操作窗口；修复上次地图视角被启动定位覆盖。并承接0.2.70的5米精度默认、用户已保存的精度值、运动朝向跟随及缺少路线终点时聚焦候选节点。具体范围见[发行说明](docs/release-0.2.71.md)。
- 新增待处理：编辑模式下地图框选卡顿；记录中照片入口优先直接拍摄，同时提供导入，并将文件夹导入选项放在入口末尾。实现完成后重跑必要检查、最终 bundle QA 并重建 APK。
- 已知基线验证：TypeScript、641项测试和网页构建通过；源代码手势模拟通过。最终打包 bundle 的交互 QA 尚待完成，且追加问题仍待实现，不能宣称本版代码冻结或最终包已验收。
- Android真机安装、覆盖安装与触控尚未验证。HarmonyOS 6.1原生HAP/APP未交付，Android APK在鸿蒙设备上的兼容性未经验证。弱信号轨迹案例仍未诊断。
- GitHub 源码提交与 Release 尚待根代理完成并核对远程 SHA/资产；不得将前述临时APK或本地构建报告为已发布。工作区含本轮源码和文档改动，尚未提交/推送。
