# 当前状态 — 2026-09-24 / 0.2.70-test 发行准备

## 2026-09-24 0.2.70-test 本地构建（待发布）

`config/product.ts` 与 `mobile/android/AndroidManifest.xml` 已提升到 `0.2.70-test` / code77；本地独立测试包已构建，发行说明见 [0.2.70发行说明](docs/release-0.2.70.md)。本版含0.2.69后尚未出包的三项改动——记录精度默认5米及范围提示、运动朝向按钮与定位跟随、缺少路线终点时聚焦节点——以及连续单指画框、明确加选/减选、双指平移/缩放和选择结果的导出/分享/删除入口。

- 精度设置仍为5–80米，既有显式保存值不重置；弱信号超过门槛时仍会拒点。弱信号案例只确认曾拒收一个估计误差10米的点，是否形成真实断轨仍未诊断，本版不宣称修复。
- APK `APK/Shantu-0.2.70-test-standalone.apk`，57,838,684字节，SHA-256 `960ba38b9e61953b3f0cac0a1fcf291263ab9df2b69321c45f07a4c4abd95621`；旁置 `.sha256` 相符。签名证书与既有测试包一致（SHA-256 `4a941b9dda8cfe6af755949ad690a5e2d4969557f99a2efe67c55637623e6f9f`），v2/v3签名及zipalign通过。包名 `com.guanyun.weather.shantu.preview`，code77；551个ZIP条目，44个Web资源条目（含32个JS/CSS/WASM文件），473张地形瓦片，入口页及地形覆盖资源存在。
- `npx tsc --noEmit`、`npm run build`、Android独立测试构建通过；根代理报告640项逻辑测试通过。网页构建有大分块和部分API路由无法静态分类的警告。最终真机/手势检查待做；本次将发布到工作分支和GitHub测试预发行版，不代表合入 `main`。
- Android覆盖安装、GPS筛点、中文输入法、双指手势和数据保留待真机验证。Android APK在HarmonyOS设备的兼容性未经验证；HarmonyOS 6.1原生HAP/APP未交付。
