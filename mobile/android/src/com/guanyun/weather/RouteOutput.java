...
final class RouteOutput {
    static String file(MainActivity activity, AppFiles files, String name, String encoded, boolean share) {
        if(name==null || !name.matches("Shantu-(route|markers|collection)-[0-9]{1,16}\\.(jpg|gpx|kml|json|xlsx)") || encoded==null || encoded.length()>12*1024*1024) return "文件过大或名称无效";
        final byte[] bytes;
        ...
        if("xlsx".equals(extension) && (bytes.length<4 || bytes[0]!=80 || bytes[1]!=75 || bytes[2]!=3 || bytes[3]!=4)) return "表格格式无效";
        activity.runOnUiThread(()->{
            if(!activity.trustedForeground())return;
            if(!share){files.saveBytes(name,mime,bytes);return;}
            new Thread(()->{
                try{Uri uri=RouteShareProvider.prepare(activity,bytes,extension);activity.runOnUiThread(()->{
                    if(!activity.trustedForeground())return;
                    try{Intent intent=new Intent(Intent.ACTION_SEND).setType(mime).putExtra(Intent.EXTRA_STREAM,uri).addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);intent.setClipData(android.content.ClipData.newRawUri("山兔文件",uri));activity.startActivity(Intent.createChooser(intent,"分享文件"));}
