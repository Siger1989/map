...
当前安装包：`APK/Guanyun-0.1.4-test.apk`，应用名“观云测试版”，包名 `com.guanyun.weather.preview`，版本代码 5。旧版签名未迁移到本机，本轮采用独立包名和新的本地测试签名，可与旧“观云”并存；不能覆盖旧版，数据也不会自动迁移。
...
8. “标记”可放地点和米制几何模型，设置颜色、名称、尺寸、方向及地表/地下位置；点选后继续编辑，长按模型可移动并同步坐标。
9. “剖面”在三维地形中按海拔水平裁切，侧边滑条或数字输入调整高度，切面可选纯色。包含本轮截面渲染、紧凑提示和拖动海拔不回跳的修复。
