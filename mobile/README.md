# 最新源码：新建、选择续画与自动存档（2026-09-08，尚未出包）

画线默认新建，继续绘制可选具体线路，完成时自动保存时间与起点附近位置；续画保留所选记录，原始实走/时间记录仍复制为手绘。说明见[绘制存档](../docs/drawing-archive.md)。新按钮采用明确底色、边框与展开/继续图标。

# 最新源码：逐点画线（2026-09-08，尚未出包）

画线固定为逐点连线，新建和续画不再切换平滑画；道路与节点吸附默认开启，仍可手动关闭。移除牵引杆设置，保留颜色与线宽。网页及安卓网页构建已通过，尚未生成包含本次更新的APK，也未完成真机触控验证。下文为历史安装包说明。
