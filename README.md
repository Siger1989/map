...
| 控制 | `modules/controls/` | 七项图层开关、影像模式、视角盘、时间轴、选点数值；可选 WebMCP 使用同一状态 |

## 数据与真实性

- **地形**：Mapzen Terrain / AWS Open Data，混合 SRTM、USGS 等来源，分辨率因地区而异。原始点高程与渲染地形因瓦片层级/插值可能略有差异；起伏增强只影响绘制。
