...
**画线与剖面入口**：底部“工具”第一行是“画线”和“剖面”，右侧不再常驻剖面按钮。“画线”进入逐点绘制与轨迹管理，开始按钮放在上方；“路线”标题右侧也可进入“画线 / 轨迹”。
...
产品已更名为“山兔”，面向全球使用。左上角移除固定成都/川西标签，首次打开进入世界地图；仍可通过定位、地名搜索或拖动地图查看目标地区。名称、导出及兼容说明见 [山兔更名与全球入口](docs/shantu-global.md)。最新安装包为 **0.2.5 独立山兔测试版**，与旧观云测试版并存、数据独立；见 [发行及压缩说明](docs/release-0.2.5-standalone.md)。
...
当前测试版 **0.2.4-test / versionCode 11**（2026-09-07）。手机优先，尽量把屏幕留给地图；原绿色模型与圆环 3D 相机控制器默认显示。当前源码已恢复矩形交线剖面，新增对象 Gizmo 和含坐标/海拔的图片导出；安装包仍以实际 Release 为准。
...
[直接下载山兔 APK](https://github.com/Siger1989/map/releases/download/v0.2.5-test-standalone/Shantu-0.2.5-test-standalone.apk) · [下载页与校验文件](https://github.com/Siger1989/map/releases/tag/v0.2.5-test-standalone) · [位置跟随说明](docs/position-follow.md) · [完整功能说明](docs/release-0.2.2.md) · [安卓安装与构建](mobile/README.md)
