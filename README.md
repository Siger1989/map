...
产品已更名为“山兔”，面向全球使用。左上角移除固定成都/川西标签，首次打开进入世界地图；仍可通过定位、地名搜索或拖动地图查看目标地区。名称、导出及兼容说明见 [山兔更名与全球入口](docs/shantu-global.md)。以下0.2.4下载仍是旧名称的已发布包，不冒充新版本。

左上角“山兔”后现显示[当前地图中心地名](docs/current-place.md)，拖动结束后自动更新，世界总览显示“世界地图”；长名称省略，服务失败时明确提示，不显示上一地区冒充当前位置。

规划路线后可直接[开始导航](docs/route-guidance.md)：显示本次已走与剩余距离、前方转向，连续可靠定位确认偏航后计算橙色接回路径；回到蓝色原路线后继续导航。原规划和途经点保留，定位差/断网有明确状态。当前为前台导航，手机真机与原签名安装包仍待验证。
