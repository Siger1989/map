...
## 2026-09-24 0.2.70-test 本地构建（待发布）

发行版本元数据已在 `config/product.ts` 与 `mobile/android/AndroidManifest.xml` 提升为 `0.2.70-test` / code77，发行说明见 [release-0.2.70.md](release-0.2.70.md)。范围为5米记录精度默认值与范围提示（保留用户已保存门槛）、运动朝向箭头/跟随联动、缺少路线终点时定位候选节点，以及连续单指加/减框选、双指地图平移/缩放和框选结果实时操作入口。

本地 APK `APK/Shantu-0.2.70-test-standalone.apk` 为57,838,684字节，SHA-256 `960ba38b9e61953b3f0cac0a1fcf291263ab9df2b69321c45f07a4c4abd95621`；`.sha256`匹配。包名 `com.guanyun.weather.shantu.preview`/code77，签名证书沿用4a94（`4a941b9dda8cfe6af755949ad690a5e2d4969557f99a2efe67c55637623e6f9f`），v2/v3签名、zipalign及473张地形瓦片核验通过。类型检查、网页构建与Android独立包构建通过；根代理报告640项逻辑测试通过。本轮目标为推送工作分支并发布GitHub测试预发行版，不代表合入 `main`。真机覆盖安装、GPS筛点、输入法和框选触控未验；弱信号报告仍未证明实际断轨，本版不声称修复。HarmonyOS 6.1原生包仍未交付，远端状态待最终核验。
