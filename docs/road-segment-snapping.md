...
底部主栏 → 画线 → 在地图上画轨迹，直接使用逐点连线；工具内也保留画线入口。新建与续画均不再提供平滑画；道路吸附和节点吸附默认开启，仍可手动关闭。点选起点和后续位置，两点之间的预览、绘制结果和保存数据都会保留道路的完整拐弯。每点一次提交一整段，撤销一次撤回这一段；绘制时只显示选定端点，不用密集的自动拐点占据地图。线条样式仅保留颜色与线宽，已移除牵引杆设置。
