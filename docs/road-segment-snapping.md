...
仍需开启道路图层、地图放大到12级以上并加载道路矢量。当前计算范围为已渲染道路。目标点有道路但找不到连通路径时，允许本段直线跨越；虚线预览和「跨越断路」提示会说明这一点，松手提交，下一个点继续沿路吸附。跨越段一次撤销，取消手势不提交。目标处没有加载可吸附道路时仍提示移动准星或放大地图；无法从纯卫星照片识别道路。跨很大范围可沿路分段点选。现有保存的直线轨迹不会自动重写；原路线规划、标记、照片、定位与本机存储格式保持。
