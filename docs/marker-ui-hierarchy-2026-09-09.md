...
当前最新候选为[三页预览v3](visuals/marker-editor-previews-v3.md)：按用户继续反馈，删除顶部重复标题，将返回、三个分组和保存合为一行。位置页每个数值包含独立回正，共6个；基本页只保留两行内容，资料继续密集排列属性和备注。整组仍待确认，v2保留为上一轮讨论，不作为最终定稿。

用户要求每项先讨论清楚，再出图。本轮按其明确指示制作[三页编辑预览v2](visuals/marker-editor-previews-v2.md)：位置以既有3D控制器为主要操作，窗口仅以X红/Y绿/Z蓝三列、位移m/旋转°两行同时提供对应数值。**不再采用平移/旋转切换，不保留大空隙或多余控制栏。** 资料页面可适当更大，属性名和内容密集同行排列，尽量同时可见。

以下较早讨论中“普通地点不显示三维控制器”“地图移动只留取消/确认”“收起全部数值面板”等建议已由最新要求替代。最终按对象类型开放的轴、数值单位与保存方式尚待细化；当前仍为视觉预览，未改代码。

状态：**只做现状核对与方案讨论。用户已确认一级摘要卡，并要求一级不需要滚动；其余层级待讨论，没有实施 UI 改动。**

最新协作顺序：每轮先出当前层级效果预览图，用户确认后才进入下一层。用户已确认[一级摘要卡预览v2](visuals/marker-level-one-preview-v2.png)：删除位于标题栏、关闭按钮左侧，首层不滚动。当前评审[二级编辑框架与基本页v1](visuals/marker-level-two-edit-v1.png)，顶部返回/保存，基本/位置/资料分组，只展开名称/图标/颜色；其余页面仍待讨论，尚未实施业务。
...
flowchart TD
  ...
  E --> K["分享 / 复制"]
  B --> L["小号删除入口：显示对象名称，再确认"]
  M["地图新建入口"] --> N["选类型 → 确定位置 → 名称/图标/颜色"]
  N --> O["创建成功"]
  O --> B
```

用户已确认一级摘要卡：图标＋名称、简短位置、海拔；下方导航、编辑、详情；最新追加标题栏小号删除入口。**一级不滚动，三个主要操作始终可见。** 名称和地址过长时截断，在详情查看完整内容；完整坐标、七级地址、属性和备注进入详情。一级不放对象下拉框、属性编辑表、旋转参数或批量导出。删除采用较小视觉内容、足够点击区域，保留关闭的独立入口。后续实现需要以390×844和360×780检查首层内容完整可见，而不是用缩小触控目标换空间。
