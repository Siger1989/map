...
最新协作顺序：每轮先出当前层级效果预览图，用户确认后才进入下一层。用户追加一级小号删除入口，当前只评审[一级摘要卡预览v2](visuals/marker-level-one-preview-v2.png)：删除位于标题栏、关闭按钮左侧，首层仍不滚动。视觉方案尚未最终确认；以下二级内容仍是之前的讨论草稿，不代表本轮开始推进二级。
