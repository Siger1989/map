# 0.2.27 记录模式、分析与批量导入 QA

2026-09-14，独立测试源127.0.0.1:9241，仅自行生成的合成文件；未触及用户原浏览器存档或请求实际GPS权限。
- 390×844：行程300×200；文件选择器multiple=true，JSON+GPX两文件预览2条轨迹；确认后收藏2项。
- 打开分析验收样例：最高区间速度4.2km/h、最大采样坡度25.1%、连续50米25.1%、陡坡1段，与纯计算一致。数据为合成样例，不是设备实测。
- 360×780：行程286×200；路线完整详情340×756，内部滚动，返回可达；页面scrollWidth=clientWidth=360。旧边框下浮窗内部有1px取整差，不形成页面横向滚动。
- 批量混入无效XML：明确“本批未导入”，无确认按钮；收藏仍2项。单文件不会在完整批次成功前写入。
- 省电模式保存、按距离开启，完整reload后仍显示“省电·按距离”。外置记录入口可打开设置，124×46且单个按钮44px，无工具遮挡。
- 原生记录未通过浏览器伪装验收；网页仅检查设置/分析/文件流。实际GPS、自动导航录制、静止硬件降频及耗电需真机。
- 首次地图有外部瓦片502后自行恢复；最终重载控制台error为空。一次重载后UI点击等待不足，读取新AX状态后正常完成，没有据此修改业务。
- 截图 artifacts/screenshots/ui-0227：map-390/360、sampling-360、analysis-390/360、batch-preview-390、batch-failure-360。最终截图已检查。
...
- 01/05：21条保存打开卡片；红绿连接3→1连续合并，795m/527m及真实DEM高度色区，路况可改。
- 02/11：头像版本0.2.25/code32和8教程，390与360内部滚动，原Logo。
- 03/09：C选中后常规6类型菜单，钻井/模型同坐标；360编辑带独立于地图。
- 04、section-10000.svg：原横版平剖图，1:10000、35.48°、全部坐标和100m钻井箭头。
- 06：地区下直接列出未分组对象，自定义组保留，未分组按钮计数0。
- 07/08：100条失败显示原因和草稿保留；删1条后同草稿成功。360绘制工具222×112，无横溢。
- 10：编辑内可直接删除，确认后模型和编辑窗口清除；整剖面删除保留并解绑两标记。
- GPX/KML由真实文件选择器导入，红绿颜色和备注一致；新页面100条和合并后备注保留，console error为空。早期开发HMR添加hooks曾报顺序错误，重新完整打开后未复现，不作为最终运行错误。
- 分享图片真实生成；12截图为桌面视口预览，不计入360验收。

浏览器检查不替代Android实际手势、键盘、保存与分享。无ADB设备，真机待验。
